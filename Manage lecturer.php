...
<?php
    ...
    else
    {
        ...
        $ln ='';
        $e ='';
        if(empty($_POST['name_lecturer']))
        {
            $errors[] = 'You forget to enter your lecturer name.';
        }
        else
        {
            $fn = mysqli_real_escape_string($con,trim($_POST['name_lecturer']));
        }        
        if(empty($errors))
        {
            $query = "INSERT INTO lecturer (name_lecturer)VALUES('$fn')";
    ...
    if(isset($_GET['id']) && !isset($_POST['update']))
    {
        ...
        $query = 'SELECT * FROM lecturer WHERE name_lecturer = '.$id;
    ...
    elseif(isset($_POST['update']))
    {
        ...
        if(empty($errors))
        {
            $query = "UPDATE lecturer SET name_lecturer ='$fn' WHERE Id_lecturer = $id";
            $r= @mysqli_query($con,$query);
            if($r)
            {                
              header( "refresh:0.1;url=Manage lecturer.php" );             
    ...
    if (isset($_GET['id']) && !isset($_POST['delete'])) {
        ...
        $id = (int) mysqli_real_escape_string($con, $_GET['id']);
        $id = is_numeric($id) ? $id : NULL;
        $query = "SELECT * FROM lecturer WHERE Id_lecturer = $id";
        ...
        if (empty($errors)) {
            $query = "DELETE FROM `lecturer` WHERE  Id_lecturer = $id";
            $r = @mysqli_query($con, $query);
            if ($r)
            {
                echo "<h1>Thank you!</h1>
    ...
    $con = mysqli_connect('localhost','root','','eva') or die('Can\'t connect to mysql server');
        ...
        if(mysqli_num_rows($result)>0)
        {
            ...
            {
                echo "<tr>  <td>$row[Id_lecturer]</td>
                            <td>$row[name_lecturer]</td>                            
                            <td><a href='Manage lecturer.php?id={$row['Id_lecturer']}'title= 'Update'>Select</a>
                            <a href='Manage lecturer.php?id={$row['Id_lecturer']}'title= 'Delete'></a></th></td>";
            }
            echo '</table>';            
        }
        else
    ...
    function printForm1($fn='')
    {
        ?>            
            ...
            <p>Name lecturer: <input type="text" name="name_lecturer" size="15" maxlength="20" value='<?=$fn?>'/></p>             
